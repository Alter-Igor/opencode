...
- **CAS bridge** (`.opencode/`): MCP `alterspective-agent` → `https://agent.alterspective.com.au/api/v1/mcp` with `CAS_MCP_TOKEN`; plugin `cas-bridge-routing.ts` injects routing/degraded prompts; safe tools `cas-safe-*` enforce agent allowlist + size caps; façades `cas-delegate` / `cas-drafter` / `cas-matter-audit`. Plan: `docs/implementation/current/FEAT-CAS-OPENCODE-BRIDGE/`. Tests: `packages/opencode/test/plugin/cas-bridge/`. Deny raw `cas_resolve_tool_call` and `cas_admin_*` via permission.
